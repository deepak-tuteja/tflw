...
// `M147-06`, closed by `M147f`, and the long spelling is back deliberately. This annotation is
// what made the defect: `verify-test-observability.mjs` used to give up on any arrow helper whose
// return type ran more than twenty characters between the parameter list and the `=>`, so the nine
// tests below resolved to `importedFrom`'s stages alone — lex and parse — and every one was
// reported as a vacuous `TF037` assertion. They are not. The workaround was to write the shorter
// `Diagnostic[]`, which is the same type; restoring the long form puts the repro back in the tree,
// where the fixed resolver has to keep handling it on every run rather than on the day it shipped.
