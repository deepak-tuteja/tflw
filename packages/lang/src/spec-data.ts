...
export const GENERATORS: readonly GeneratorEntry[] = [
  ...
  { id: 'transform-base64', family: 'transform', syntax: '`base64 encode(...)` / `base64 decode(...)`', notes: 'pure deterministic value transform, not a fresh-value generator (M18)', example: '`base64 encode("{email}:{password}")`' },
  { id: 'transform-hex', family: 'transform', syntax: '`hex encode(...)` / `hex decode(...)`', notes: 'pure deterministic value transform, not a fresh-value generator (M18)', example: '`hex encode("{token}")`' },
  { id: 'transform-length', family: 'transform', syntax: '`length of <value>`', notes: 'the length of a string or a list — what `.length` reads; binds tighter than arithmetic (`M242`, `D1329`)', example: '`let n = length of {items}`' },
  { id: 'transform-join', family: 'transform', syntax: '`<list> joined with <separator>`', notes: 'a list of strings and numbers as one string; binds loosest of all (`M242`, `D1329`)', example: '`let csv = {ids} joined with ","`' },
  { id: 'transform-url', family: 'transform', syntax: '`url encode(...)` / `url decode(...)`', notes: 'pure deterministic value transform, not a fresh-value generator (M18)', example: '`url encode("{query}")`' },
] as const;
...
export const STEP_KEYWORDS: readonly StepKeywordEntry[] = [
  ...
  { id: 'accept', family: 'browser', syntax: '`accept dialog` / `accept dialog with <value>`', summary: 'arm a handler accepting the *next* native dialog; armings queue, one dialog each, in order (`D797`); without one Playwright auto-dismisses silently. `with` types an answer into a `prompt` and interpolates (`D800`); reaching a kind that takes no answer is `TF080`, and an arming no dialog ever consumes is `TF079` — both runtime warnings (`D801`, `D802`)', example: '`accept dialog with "Blue"`' },
  { id: 'dismiss', family: 'browser', syntax: '`dismiss dialog`', summary: 'arm a handler dismissing the next native dialog; armings queue, one dialog each, in order (`D797`); an arming no dialog ever consumes is `TF079`, a runtime warning (`D802`)', example: '`dismiss dialog`' },
...
export const CONFIG_KEYWORDS: readonly ConfigKeywordEntry[] = [
  ...
  { id: 'authorized', slot: 'key', summary: '`authorized target "<url>" reason "<text>"` — written permission for the security scans to probe that target (D21); `authorized target env NAME default "<url>" reason …` takes the origin from the environment variable when set' },
...
const DIAGNOSTIC_ROWS: readonly Omit<DiagnosticEntry, 'example'>[] = [
  ...
  { code: 'TF026', meaning: 'Checker: an `api <service>`/`wait until api <service>` name not declared in the active env — checked in test/action/hook bodies **and** inside `session` blocks (P#66).', probes: [{ wrap: 'step', source: ['api billng POST /auth/login'], says: 'did you mean `billing`?', needs: {'services':['billing']} }] },
  ...
  { code: 'TF030', meaning: 'Checker: a `{var}`/bare-identifier reference provably never bound anywhere reachable in its scope — conservative (P#57): only flags a name that\'s *definitely* unreachable, never one that merely might be.', probes: [{ wrap: 'step', source: ['api POST /orders', 'capture body.ok as orderId', 'api GET /orders/{orderid}'], says: 'unknown variable "orderid"' }] },
  { code: 'TF031', meaning: 'Checker: a `request` assertion (`connects`/`fails`) combined with a response-based assertion (`status`/`header`/`body`/`duration`) on the same request, or used at all inside `wait until api` (`PLAN_ENTERPRISE.md` decision 18).', probes: [{ wrap: 'step', source: ['api GET /a', 'expect request connects', 'expect status equals 200'], says: 'can\'t be combined with `request connects`/`fails` on the same request' }] },
  { code: 'TF032', meaning: 'Checker: an `upload … type "…"` value that is a non-interpolated literal not shaped like `type/subtype` (M19) — a light regex, not an IANA vocabulary check, so it only catches an obvious typo before the run.', probes: [{ wrap: 'step', source: ['api POST /u upload "./f.png" as "avatar" type "imagepng"'], says: 'invalid content type "imagepng", expected a "type/subtype" shape like "image/png"' }] },
...
export const CLI_FLAGS: readonly CliFlagEntry[] = [
  ...
  { flag: '`--tag <name>[,<name>...]`', command: 'run', effect: 'only runs tests carrying any of the listed `@name`s (comma-separated OR; combines with `--only` as AND); a `!` excludes — `--tag !slow` runs everything not tagged `@slow`, exclusions AND together and beat an inclusion (`M242`, `D1327`)' },
  ...
  { flag: '`--shard <i>/<n>`', command: 'run', effect: 'runs every nth file of the sorted discovered suite, starting at the ith (`M242`, `D1330`) — the n shards are disjoint and together the whole suite, so a CI matrix of n jobs runs everything once; `--tag`/`--only`/`--failed` then narrow within the shard, and every shard still validates the whole suite first' },
  ...
  { flag: '`--allow-public-target <origin>`', command: 'check', effect: 'the same affirmation `run` takes, accepted here because `check` answers *will this run?* and after M131a the answer depends on it — without it a suite legitimately scanning a public staging host could never get a clean `tflw check` (D345). `check` still sends nothing: the flag only changes which diagnostics it reports' },
...
export const DECLARATIONS: readonly DeclarationEntry[] = [
  ...
  { id: 'tags', group: 'header', syntax: '`@<tag>` on its own line(s) above a `test` or `crawl`', summary: 'label a declaration for `--tag` selection — OR across a comma-separated list, and there is no exclusion flag (`SPEC` §4.1); a tag line is shared prefix, so which construct it introduces is only knowable past the tags (`D450`)', example: '`@smoke @checkout`' },
  ...
  { id: 'skip', group: 'header', syntax: '`skip "<reason>"`', summary: 'keep the test in the file and run none of it — reported as its own outcome with the reason; a blank reason is `TF084` (`M242`, `D1327`)', example: '`test "refunds settle" skip "the payments sandbox is down until the 3rd"`' },
