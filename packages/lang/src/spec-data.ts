...
const DIAGNOSTIC_ROWS: readonly Omit<DiagnosticEntry, 'example'>[] = [
  ...
  { code: 'TF022', meaning: `Parser (config): top-level config content that isn't one of ${listConfigDirectives()} (M110, \`V4-04\` — this list is \`CONFIG_DIRECTIVES\` above, the same array the parser's own message is built from, so the two cannot drift again). The code carries a **second** use this row went two arcs without naming: a \`session … oauth2\` block that is missing a required line. \`M147e\` (\`A2-15\`) found it while narrowing that message — the block used to name all three of \`token url\`, \`client id\` and \`client secret\` however many were already written, and now names only the ones absent — and the omission here is the same doc-drift class as \`A2-16\`, caught in the file that generates \`diagnostics.md\`.`, probes: [{ wrap: 'config', source: ['workers 3'], says: 'expected `defaults`, `env`, `session`, `require`, or `exclude`, found `workers`' }] },
