...
const DIAGNOSTIC_ROWS: readonly Omit<DiagnosticEntry, 'example'>[] = [
  ...
  { code: 'TF029', meaning: 'Checker (config): a duplicate `session` name.', probes: [{ wrap: 'config', source: ['session admin', '  api POST /login', 'session admin', '  api POST /login'], says: 'duplicate session `admin`', as: 'two `session admin` blocks in one `tflw.config`' }] },
