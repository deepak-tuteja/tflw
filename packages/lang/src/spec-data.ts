...
export const DIAGNOSTICS: readonly DiagnosticEntry[] = [
  ...
  { code: 'TF034', meaning: 'Checker (load, M43/D70): a `threshold … for "label"` clause references a label that matches no `api` step\'s identity (its explicit `as "label"` tag, or its automatic `METHOD path.raw` identity when untagged) within the same scenario.', example: '`threshold p95 duration for "checkotu" is less than 250ms` with only an `as "checkout"`-tagged step in scope → `threshold for "checkotu" matches no step in this scenario`' },
] as const;
