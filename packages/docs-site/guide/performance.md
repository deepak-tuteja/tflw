...
| --- | --- |
| [Load testing: workloads & scenarios](/guide/load-testing) | how do I generate load? |
| [Thresholds, results & validation](/guide/load-results) | how do I judge what came back? |

Two chapters, split at that seam. The reason this pillar has an overview as well is the paragraph
below: the relationship between the two *execution models* — functional and workload — is the thing
readers get wrong, and it belongs somewhere before the chapters that assume it.
...
- **`expect` still means `expect` under load.** Correctness does not stop being checked once a
  ...
  [the thresholds chapter](/guide/load-results#thresholds-—-the-pass-fail-gate).

## Where to go next

- **Writing your first load test:** [Load testing: workloads &
  scenarios](/guide/load-testing) — the five workload shapes, `pause`, `parallel` and `--workers`.
- **Deciding whether it passed:** [Thresholds, results &
  validation](/guide/load-results) — threshold rules, the four exit codes, and the run that
  refuses to give a verdict.
- **Comparing it against what you already run:** that chapter's [measured comparison against
  k6 and Artillery](/guide/load-results#validated-against-k6-and-artillery).
