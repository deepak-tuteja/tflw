...
/**
 * `packages/ui/src/Wordmark.tsx` — the EIGHTH output, and the only one that is not a file the
 * browser loads directly (`M233` `H`, `D1287`).
 *
 * The page it goes to has four themes, and `styles.css`'s own header is the reason this is
 ...
 * cannot see the host page's custom properties, so `currentColor` and `var(--accent)` resolve
 * against nothing. Inline is the only shape in which `D1286` is expressible at all.
 *
 * **Why generated.** Hand-copying `WORDMARK.ink` into a second repository location is the exact
 * drift this script's own docblock exists to prevent, and a `.tsx` in a typechecked package is
 * the copy most able to rot unnoticed. `brand-mark.test.mjs` holds this output against this
 * function, which is the first drift guard the mark has had in either direction.
 *
 * `role="img"` + `aria-label` are carried rather than added: `wordmarkSvg` already emits them,
 * and replacing a text node with a picture otherwise deletes an accessible name (`D1289`).
 */
export function wordmarkTsx() {
  const [, , w, h] = WORDMARK.viewBox.split(' ').map(Number);
  const aspect = w / h;
  return `// GENERATED by packages/docs-site/scripts/generate-brand-assets.mjs — do not edit by hand.
...
// tokens (\`D1286\`).
//
// \`height\` is the only dimension asked for; the width follows the viewBox's ${aspect.toFixed(3)}
...
export function Wordmark({ height = 40, className }: WordmarkProps) {
  return (
    <svg
      ...
      aria-label="tflw"
      height={height}
      width={height * ${aspect.toFixed(5)}}
      className={className}
      fill="none"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <g transform="${WORDMARK.shift}">
        <g stroke="currentColor" strokeWidth={${WORDMARK.inkWidth}}>
${WORDMARK.ink.map((d) => `          <path d="${d}" />`).join('\n')}
        </g>
        <path d="${WORDMARK.rail}" stroke="var(--accent)" strokeWidth={${WORDMARK.railWidth}} />
      </g>
    </svg>
  );
}
...
async function main() {
  ...
  const vectors = [
    ...
    // The eighth (`D1287`). Not under `PUBLIC` and not an SVG: it is source, in another
    // workspace, and it is here because the geometry above is the only copy of this mark.
    [join(REPO_ROOT, 'packages', 'ui', 'src', 'Wordmark.tsx'), wordmarkTsx()],
  ];

  for (const [path, svg] of vectors) {
    if (!svg.trim()) throw new Error(`${path}: empty output`);
    // `wordmarkTsx` ends in its own newline; the SVG builders do not. Appending unconditionally
    // left the generated component with a trailing blank line that no editor keeps, so the sync
    // gate would have gone red on the first save of an untouched file.
...
// Guarded so `brand-mark.test.mjs` can import the geometry and the builders without launching a
// browser or writing a file (`D1287`). Run directly, nothing changes.
