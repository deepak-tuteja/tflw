...
export function renderReportHtml(report: RunReport, assetHrefs: ReadonlyMap<string, string> = new Map()): string {
...
<main>
${slots.map((s) => renderTest(s, s.id === defaultActiveId, assetHrefs)).join('\n')}
</main>
</div>
...
function renderTest(slot: TestSlot, active: boolean, assetHrefs: ReadonlyMap<string, string>): string {
  ...
  ${priorAttempts.map((a) => renderAttempt(a, assetHrefs)).join('\n')}
  ...
  <ol class="steps">
${test.steps.map((s) => renderStep(s, assetHrefs)).join('\n')}
  </ol>
</section>`;
}
...
function renderAttempt(attempt: AttemptResult, assetHrefs: ReadonlyMap<string, string>): string {
  return `<details class="attempt">
    ...
    <ol class="steps">
${attempt.steps.map((s) => renderStep(s, assetHrefs)).join('\n')}
    </ol>
  </details>`;
}
