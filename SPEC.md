...
| `TF033` | Parser/checker (load, M29/M30): a `scenario`'s workload/threshold shape is invalid, two `scenario`s in one file share a name (M30, D29 — names key each scenario's own metrics/threshold breakdown under concurrent multi-scenario runs), a browser step appears inside a `scenario` body (D19 — API-only in v1), or `think` appears outside a `scenario` (D18). | `think 2s` inside a plain `test` → ``think` is only legal inside a `scenario` (SPEC §2)` |
| `TF034` | Checker (load, M43/D70): a `threshold … for "label"` clause references a label that matches no `api` step's identity (its explicit `as "label"` tag, or its automatic `METHOD path.raw` identity when untagged) within the same scenario. | `threshold p95 duration for "checkotu" is less than 250ms` with only an `as "checkout"`-tagged step in scope → `threshold for "checkotu" matches no step in this scenario` |
