...
- File-backed: `with each from "./data/invites.csv"` (also `.json`). Columns bind by header
  name; the checker verifies the file exists (`TF043`, M97c). It does **not** check that the
...
| File-backed | `body from "./payloads/order.json"` | file is a template — `{vars}` interpolate; checker verifies existence (`TF043`, M97c) when the path itself is a literal |
...
- **An action may not reach itself** — not directly (`a` calling `a`) and not through others
  (`a → b → a`). Rejected by the checker (`TF044`, M97d) whenever every action in the cycle is
  declared in one file, and by the runtime otherwise, which reports the same path in the same
...
| Command | Purpose |
|---|---|
| `tflw init [--load]` | scaffold `tflw.config` + `example.tflw` + `.env.example` + `.gitignore` (`.env`/`report/`, appended without duplicating if the file already exists) — decision 82; API-only, `--ui` is M3. `--load` (M29/D30) additionally scaffolds a `load.tflw` holding a workload-bearing `test` in the open (`rps`) model, run by plain `tflw run` like any other file |
