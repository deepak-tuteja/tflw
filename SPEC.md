...
- Teaching-quality diagnostics are a feature: every **checker** error carries source line, caret,
  and a "did you mean" where possible (P#6). A **runtime** failure carries a message and, where one
  applies, a hint — but no caret: `RuntimeError` (`packages/runtime/src/eval.ts`) holds a message,
  an `actionPath` and a `rootMessage`, and no span, so there is no source position to point at.
...
coded.** All of `packages/runtime/src` assigns `code:` exactly once, and that one is the checker's
`TF043` file-existence pass (`imports.ts`, `checkConfigFiles`) — not something a test hits mid-run.
Narrowed here in `M144b` (`V4-12`); the gap itself is tracked as `M144-02`.
>
...
**A key declared twice in one block is `TF081`, an error** (`M165a`, `D829`). Entries are applied in
order and most of them assign, so a second `timeout step` overwrites the first and the earlier line
is read, discarded and never mentioned — which is the same silence `TF024` and `TF029` remove for
duplicate env and session *names*, one level down at the key. Four keys are exempt because a second
declaration of them **adds** rather than replaces: `header`, `allow hosts`, `authorized target` and
`redact`. Two are keyed finer than their leading word, because the resolver stores them in a map:
`timeout step` does not collide with `timeout expect`, and `api` does not collide with
`api payments`. **The rule is per block.** Setting a key in `defaults` and again in an `env` is the
reason both blocks exist, and the env's value winning there is the same last-one-assigns behaviour —
so it is reported inside one block and never across two.
...
- **Every `env(NAME)` must be declared** (`TF077`, M156a). The startup gate above checks the names
  ...
  in the source and the declaration is in the config, so neither can change between check and run.
  The two rules compose into the guarantee, and neither gives it alone — every reference is
  declared (check time), every declaration is satisfied before the first request (run start).
  The converse is deliberately *not* a rule: a declared name nothing reads stays silent forever,
  because a secret that only has to exist for the redactor to pre-register it is an ordinary shape.
- **Whether a variable is *set* is a different question, and `tflw check` answers it without
  failing.** When a declared variable is absent from both the environment and `.env`, `check`
  prints one advisory line — `ℹ require env: 2 of 10 not set here (ADMIN_PW, OAUTH_CLIENT_SECRET)`
  — and still exits 0, because a secretless lint job and a run with secrets are different
...
- **`env(NAME)` is a value, not an interpolation.** This language interpolates `{variable}` only,
  and `env(…)` is not path syntax — so `"{env(TOKEN)}"` inside a string is *literal text*, ships
  those twelve characters over the wire, and fails as a 401 rather than as a missing variable. It
  is `TF078`, a warning (M156b). Written out because every reader who has met `{var}` guesses
  wrong:

  ```
  header "Authorization" is "{env(TOKEN)}"          # wrong — sends the braces
  header "Authorization" is env(TOKEN)              # right
  header "Authorization" is join("Bearer ", env(TOKEN))   # right, with a prefix
  ```
...
- **Every scannable origin, not just the default `api` base** (M131a, D343). A step naming a service
  ...
  class of destination, same `TF060`.

#### 3.10.1 Public targets — `--allow-public-target` (M131a, D21/D340) ✅

```
tflw run --allow-public-target https://stg.example.com
tflw check --allow-public-target https://stg.example.com   # same flag, same meaning
```

`authorized target` above is the *declaration* layer, and it lives in `tflw.config` — a file you can
merge into `main`. This is the layer that makes a committed config unable, on its own, to point a
scanner at the internet. **It has no config key, and it never will**: a key granting it would not be
a feature, it would be the deletion of this control.

- **Required for scans that *originate* traffic.** Today that is `expect response has no
  authorization violations` (§6.2's matcher table; `TF060`/`TF062`–`TF064`), which re-issues the
  observed request under every other declared principal. Missing flag is `TF065`, both a checker
  error and a run-time refusal.
- **Not required for `security violations` (§9.10).** That scan only inspects a response the suite
  already asked for under `allow hosts`, so there is no extra packet to authorize. Gating both
  uniformly would read simpler and would predictably end with the flag parked permanently in CI, and
  a control everybody leaves on is not a control.
- **Address class is read from the URL as written — no DNS, ever.** A control that resolves a name
  sends a packet to decide whether it may send a packet; the answer differs on a VPN, differs
  between a laptop and CI, and a TTL-0 record can rebind between the check and the probe. Exempt:
  loopback (`127.0.0.0/8`, `::1`), RFC1918, IPv6 unique-local (`fc00::/7`), link-local, CGNAT
  (`100.64.0.0/10`), and the name `localhost` (and `*.localhost`). **Every other hostname is
  public**, including a genuinely private `api.internal.corp` — nothing in that string says so, and
  the control is built to over-ask rather than under-ask. `0.0.0.0` and `::` are neither: they name
  no host, so they are refused rather than exempted or affirmable.
- **Origin-valued and repeatable**, one origin per occurrence, no comma-separated form and no
  wildcard. A bare boolean would affirm a *category* and survive any later edit of the config,
  silently covering whatever new host somebody pointed the suite at — which is most of what this
  layer takes away from config. It must also match an `authorized target` this env declares: the
  flag is additive on top of the declaration, never a way around it. A flag matching nothing this
  run would scan is `TF066`, not a no-op.
- **No `--reason`.** The reason lives on the declaration, where it travels into the report artifact.
  A second one on the command line could only duplicate or contradict it, with no defined winner.
- **Two doors, and the run-time one is authoritative.** `tflw check` refuses what it can prove
  before any server or credential is involved — but the env's base URL is interpolation-resolved
  *on the machine running the check*, so a clean `tflw check` on a laptop is not a promise about CI.
  The probe engine judges the origin the request is actually going to, and reuses `TF065` because
  the repair is the same one.
- **Probe pace.** Probes are issued strictly sequentially, one in flight per assertion, in declared
  order with `anonymous` last. That bound is asserted by a test rather than left to hold by
  accident; a *declared* pace (`probe rate` or similar) is deferred until the first change that
...
### 3.12 Helper directories — `helpers` (M239, D1319)

```
helpers "./helpers", "./lib/tflw"
```

- Top-level, like `exclude` (§3.9): one or more directories, relative to this `tflw.config`'s own
  directory, that a `use` (§11) may load a JS/TS module from. A config that declares none allows
  `./helpers` and `./tests/helpers`.
- **Judged by the checker, as text.** A `use` path resolves against the directory of the file that
  names it — the runtime's own rule — and the result must sit inside one of the allowed
  directories. `tests/api/a.tflw` writing `use "../../helpers/x.ts"` lands in `helpers/` and is
  fine; `use "../lib/x.ts"` from `tests/` lands in `lib/` and is `TF083`, an error, because this
  is the one declaration that decides what code a run executes. No `stat`, no symlink following:
  the checker keeps its no-I/O rule, and a module that is *missing* is still `TF043`.
- `tflw check` prints one `helper <module> — \`use\` in <files>` line per module the checked
  files name, before its verdict, so a reviewer sees the executable surface of a suite without
  opening it. `tflw run --no-helpers` reports every `use` as `TF083` naming the flag and starts
  nothing.
...
| browser | `accept` | `accept dialog` / `accept dialog with <value>` | arm a handler accepting the *next* native dialog; armings queue, one dialog each, in order (`D797`); without one Playwright auto-dismisses silently. `with` types an answer into a `prompt` and interpolates (`D800`); reaching a kind that takes no answer is `TF080`, and an arming no dialog ever consumes is `TF079` — both runtime warnings (`D801`, `D802`) | `accept dialog with "Blue"` |
| browser | `dismiss` | `dismiss dialog` | arm a handler dismissing the next native dialog; armings queue, one dialog each, in order (`D797`); an arming no dialog ever consumes is `TF079`, a runtime warning (`D802`) | `dismiss dialog` |
...
The `prompt` row is the one the arming can carry an argument for (`D800`):

```tflw
accept dialog with "Blue"                # the answer typed into the prompt; interpolates
click button "Set favourite colour"      # `with {colour}` works the same way
expect dialog type equals "prompt"
```

`accept dialog` with no `with` answers the empty string, which is what it has always done.

The other three kinds have no input to fill, and Playwright discards `promptText` for them without
a word. tflw does not inherit that silence: an answer reaching a non-`prompt` raises **`TF080`**, a
**warning** at run time (`D801`), and the dialog is still accepted. It is not an error because the
kind is not knowable before the page raises it — a page that raises a `prompt` or a `confirm`
depending on state is a legitimate thing to test, and a static rule would have to be wrong about it.
**An arming that no dialog ever consumes raises `TF079` (`D802`).**
...
If nothing raises a dialog before the test ends, the arming did nothing: the test proves exactly
what it proves with the line deleted, which is the shape §6.4 refuses for assertions and this
refuses for armings. One warning per unconsumed arming, at that arming's own line. A surplus
*dialog* is not a surplus arming — one arming and two dialogs leaves the queue empty, and the
second dialog takes the dismiss default (§9.1 above), so nothing is reported.

`TF079` is reported **only on a test that otherwise passed**. An absence has two causes and the run
cannot tell them apart: the arming was dead, or the test failed before reaching whatever would have
raised the dialog. On a failing test the second is the likely one, and a warning there would point
at an innocent line on every failing test that happens to arm a dialog first. `TF080` carries no
such condition, because it reports a dialog that did fire.

`TF079` and `TF080` are the only two codes in §17 that `tflw check` cannot emit, and the only two
whose rows carry a runtime test in place of a probe.

`beforeunload` is raised only after a real user gesture, so a test expecting one may legitimately
see none — and is therefore the one kind for which `TF079` may be reporting a page's rule rather
than a test's mistake.
...
**A documented path resolves against the *document's* server, not against the `api` base's path**
(`M137c1`, `D480`). tflw reads `servers[0]` — absent, `[]` and `/` are one case per the specification —
and joins the document's declared prefix onto the **origin** the `api` base names; an absolute
`servers` url contributes its path and never its host, because which deployment is under test is the
...
either door. **And when the surface resolved but nothing landed** — requests sent, none reached
(`D481`) — for the same reason read from the far side: the body judged no response, so a green run
...
| `tflw spec [--json]` | print the construct manifest of *this build* — every declaration, step keyword, matcher, generator, locator, `tflw.config` word and diagnostic code the parser dispatches, plus a build stamp (version, commit, dirty, build time). M154a, D736-D738; the `declaration` family is M154c, D742. **It exists for conformance testing.** `testFlow-tests` is this language's target and had no way to ask what the surface *is*: the published package is one self-contained bundle with no `@tflw/*` packages to import, so a coverage gate there cannot read `STEP_KEYWORDS` out of a module, and the cost of that was measured — seven step keywords with **zero** occurrences across 126 `.tflw` files, and four of the six workload shapes never executed by anything. The manifest lists **what this binary dispatches and only that**: a 🔮 planned construct is *absent* rather than listed as planned (D736), so building one makes it appear and a downstream gate turns red on its own, with nobody remembering to flip a badge — `MATCHERS` is the one table carrying its own status field and is emitted verbatim. Every source table is held two-way to the parser (`STEP_KEYWORDS`/D277, `CONFIG_KEYWORDS`/D444, and `LOCATORS`, which joins them here), so the manifest cannot offer a word the parser rejects nor omit one it accepts. **`DECLARATIONS` joins them in M154c and is held there *behaviourally***: the seven top-level words and five `test`-header clauses are each parsed as a minimal file and the parser asked what it did, because two arrays agreeing only proves someone edited both. That family was missing from M154a and the gap was structural rather than cosmetic — `retry` and `after` hooks are constructs M154c plants, and a gate cannot demand what it cannot see, which is the same argument M154a recorded for adding generators. Diagnostics contribute their **codes** and none of §17's prose — ~78 KB of markdown written for a table, in a document a gate reads every run. The **build stamp** is the other half and closes nothing on its own: `commit` is a short sha or `null` where there was no git to ask (a published tarball, a vendored checkout), never invented, because an invented one would be believed; `dirty` is `null` when there is no commit for it to be relative to; `source` is `dev` under the unbundled `npm run dev`, where the esbuild `define`s do not exist. `--json` rather than `--format json` (D738): `run`/`check` name a member of an open set of renderings, `spec` has one machine form and one human one |
...
| `tflw record <url> [--browser chromium\|firefox\|webkit]` | opens a real, visible browser at `<url>` and prints **one tflw step per action taken in it** — `pick`'s sibling and deliberately a second command rather than a flag on it: `pick` is *inert*, calling `preventDefault`/`stopPropagation` so that identifying an element does not navigate or submit, and recording needs the opposite, because a recording is a sequence and a sequence needs the page to advance. Locators are resolved the same way `pick` resolves them — the runtime's own tiers (D6), confirmed to name exactly the element acted on (D7), with a generated CSS selector as the fallback. A field's keystrokes are coalesced into one `fill`; a checkbox prints `tick`/`untick` rather than a fill with the string `"on"`; a `<select>` prints `select`; a key press carries the control it was typed into, since a page-level `press` goes to whatever has focus and *whatever has focus* is a fact about the moment rather than about the test. A navigation prints `open` only when no action preceded it — a click that navigates is already written down as the click. A click that opened a **tab** prints the `switch to new tab` block that wraps it, because the runtime must be listening before the trigger runs; every tab in the session is recorded, and returning to an earlier one prints `switch to tab N`. A name that matches more than one element is scoped by the `within` that makes it unambiguous — the nearest ancestor that narrows it to one **and** has a name of its own — rather than surrendered to a structural CSS path. A click the browser synthesised, such as the one an implicit form submit dispatches at the submit button, is not recorded; keyboard activation of a focused control is. **Actions only, never expectations**: a recorder sees that a page changed, not what about the change mattered, so assertions are authored afterwards. Every line is built through the language's own builders and printed, never concatenated, because every value in it is content the page supplied. **stdout carries steps and nothing else** — the banners and the word it writes when it skips an action go to stderr, so a redirect gets a file whose every line is a step. Runs until the window is closed or Ctrl+C; `<url>` must be absolute (no `tflw.config` involved) |
...
| `tflw migrate [files] [--env E] [--no-color]` | mechanically rewrite a suite past every deprecation the checker can name (P#38/45, P#112; cluster C8/M90). A diagnostic carrying a `deprecation.replacement` gets that exact source span spliced, widest-first, in the same ordering `refactor apply` uses — no id to pick, since a deprecation is never something to leave half-migrated on purpose. Three rules carry one today: `scenario`→`test`, `think`→`pause`, `uncheck`→`untick`. **Bare `check <locator>` deliberately carries none** — `tick field "…"` (the old click) and `check field "…" is checked` (the assertion) are both honest readings of it, and guessing wrong writes a mutation into a test that keeps passing, so migrate reports it and leaves it to you (§9.1). Any diagnostic that offers ``run `tflw migrate` to apply this automatically`` is one it can act on; that line is *derived* from the payload, so the offer and the capability cannot drift apart. **It acts on files that do not parse** — the only kind it exists for: it splices what it can, writes, re-checks the rewritten source, and renders whatever remains against post-splice offsets, repeating until a pass finds nothing left (a `think` nested in a `scenario` is invisible to the parser until the `scenario` is fixed). Exit **0** when the suite is clean afterwards, **2** when errors remain — including when migrate successfully did work and the file still fails, because migrate's job is the rewrite, not the verdict. It rewrites *keywords*, not prose: a migrated file can be entirely correct and still say the old name in its comments and `test "…"` names. Takes no `--format`: its output is a report of edits, not a diagnostics array (`check --format json` already serializes `deprecation` for machines). The second of the three commands that mutate source (`refactor apply`, this, and `fmt`) |
| `tflw export otlp [report-dir] --endpoint <url> [--header <name=value>]` | send a finished run to an OpenTelemetry collector as one trace (`M242`, `D1331`): a span for the run, each file, each test and each step, over OTLP/HTTP JSON with no SDK. Reads `results.json`; the spans are laid end to end from the run's `startedAt` because no artifact records when a test started, and every span carries `tflw.timing = "reconstructed"`. Ids are hashed from the run, so a re-export is the same trace. A failed test or step is an `ERROR` span with its message; a skipped test is `OK` with `tflw.outcome = "skipped"`. Exit 0 on a 2xx, 1 otherwise, 2 for usage |
| `tflw fmt [paths] [--check]` | format `.tflw` files in place (`M191`, `D994`–`D997`): a function over the lexer's token stream, never the AST — the same tokens come out in the same order and only the whitespace between them is decided. Two-space blocks; one space between tokens, none before `,` `:` `)` `]` or after `(` `[`; objects padded `{ a: 1 }`, arrays `[1, 2]`, interpolations `{name}` tight by the `{ IDENT }` rule (§4); a multi-line bracket body one level past its opener's line with the closer at that line's indent; data tables aligned by column; blank lines collapsed to one, trailing whitespace removed, one final newline. **Comments never move relative to code** and their text is never touched: a comment line takes the next code line's indent (or its block's, at the end of one), a trailing comment gets two spaces before `#`. Three spaces are kept as written because the parser alone tells the two readings apart: after `-`/`+` (unary vs binary), between a number and the word after it (`4s` vs `3 seconds`), and between a name and `[` (`body[0]` vs `equals [1, 2]`). A path may be a directory (walked; `node_modules/`, `.git/`, `report/` skipped); none means the cwd. A file the lexer refuses is named, left alone, and exits 1. `--check` writes nothing, lists what would change, exits 1 if anything would — the CI form. The guarantee, gated in CI over this repository's own files (`verify:fmt-roundtrip`): same tokens in the same order with trivia excluded, every comment kept, the indent/dedent structure unchanged, idempotent. The same function serves `textDocument/formatting` through `tflw lsp`. The third of the three commands that mutate source, and the only one that cannot change what a file means |
| `tflw ui [dir] [--port N] [--no-open]` | serve the page for a project on `127.0.0.1` (`M192`, `D985`–`D988`): the discovered files and their tests; a run started from the page as `tflw run --format ndjson` — the same command and the same report directory a terminal run uses — with its stream relayed live; and every report directory the project holds, each run from the page kept aside as `report/runs/<id>/` when it ends. The page is a projection of the file and the artefacts, never a second format, and nothing in it writes a `.tflw` file. Loopback only, by decision — reach it from elsewhere over `ssh -L` |
...
| `TF010` | Parser: a token appeared where the grammar didn't allow it (the catch-all "unexpected token" code — covers many distinct shapes: a missing path after `api GET`, a multi-word call missing its parens, a malformed table row cell count, etc.). Its trailing-token form names what just ended, and that noun follows the dialect (`M147e`, `A2-13`): **step** inside a test body, **directive** in `tflw.config`, **declaration** at a `.tflw` file's top level. It said `at end of step` in all three until then, including in a file that has no steps in it anywhere. The hints it offers follow the same rule — the ones that name `expect`, `api` requests or inline `within` are step-only, while the duration-adjacency hint is offered in every dialect because adjacency is a lexical rule. | `api GET` → `` expected a path like `/orders`, found end of line `` |
| `TF011` | Parser: an unrecognised statement keyword where a step was expected, or a *retired* one — a keyword the parser still recognises solely so it can name its replacement outright (FS-04's `uncheck` → `untick`, D103 style). A retired spelling is kept out of both the did-you-mean vocabulary and the "expected one of" fallback: offering it back as valid would be worse than no suggestion. Carries a third shape since `M147e` (`A3-19`): a run of two or more words filling the whole line and reaching no `(` is read as a call missing its parens and told so, rather than being handed the step-keyword enumeration. That is the same answer `TF010` gives the same mistake in **value** position (D168) — the codes differ because each names the production that raised it, and the wording no longer does. A near-miss keyword still wins, so `expct status equals 200` stays a typo; the enumeration survives for a single unknown word, where it is the complete answer to what may go there. | `expct status equals 200` → `` did you mean `expect`? ``; `uncheck field "Terms"` → `` `uncheck` was renamed to `untick` `` |
...
| `TF015` | Parser: a block that requires an indented body does not have one — eleven rules share it (`test`, `action`, hooks, `session`, `session … oauth2`, `crawl`, `within`, `download`, `switch to new tab`, `fill form`, a `with each` table, and a `defaults`/`env` block in `tflw.config`). **Anchored on the construct's own header line since `M147e` (`M106-02`).** Each of these fires after the header has been read, and most of them used to anchor on whatever the cursor then sat on — which is the *next* construct, or, where the rule fires past a consumed dedent, the line after the whole block. `this `before file` has no steps` put its caret under the word `test` on a later line: a declaration with nothing wrong with it. `M106` compensated for this in the renderer and `M106-02` said only the production knows which token its sentence is about, which is now where the span comes from. The anchor is always one line, so the CLI caret and the LSP's squiggle range agree about extent as well as position. | a `before file` block with no steps under it → `` this `before file` has no steps `` |
...
| `TF060` | Checker (M128b, D291): `expect`/`check response has no … security violations` written against an env whose `api` base URL no `authorized target` declaration names. D21's declaration layer, made load-bearing in the milestone that introduces it — the alternative, ship the grammar and enforce it once something actually sends a probe, means shipping a safety control nothing exercises, which is the same criticism relocated. Matching is by **origin** (scheme + host + port), not by the pattern rules `allow hosts` uses: a declaration for `https://x.example.com` does not authorize `https://x.example.com:8443`, which is a different listener that may belong to a different team. **Loopback is not exempt**, deliberately — exempting it would exempt exactly the target this arc is tested against, shipping the requirement untested. **M131a/D343 widened it from the default `api` base to *every scannable origin*** — the default base plus each declared service — because `api @billing GET /invoices` reaches a different host, and a scan there was gated by nothing at all. Same repair (add a declaration naming the origin), so the same code, which is this codebase's rule that a code is one repair rather than one topic. Still narrow in the two ways that remain: only origins the env itself declares, never an absolute URL a step writes for itself, and only fully literal ones — `api "https://{API_HOST}/v1"` is skipped, not guessed at, and skipped entirely when no config was resolved. | `api GET /orders` then `expect response has no security violations` → `` needs an `authorized target` declaration ``; `api GET /orders` then `expect response has no security violations` → `` service `@billing` `` |
...
| `TF075` | Parser (`M147e`, `A3-14`, D643): **input nested more deeply than the parser will descend.** `parseSource` is documented as never throwing for a syntax error, and a file of 30 000 unary minuses broke that outright — a raw V8 `RangeError`, exit 2, no filename, no line, no caret. Not a bad diagnostic but *no* diagnostic, and a stack trace where a caret should be. Unary minus is the only production in this grammar that recurses per token rather than looping: `+ - * /` chains iterate, `within` nesting is bounded by the lexer, and a JSON body goes through `JSON.parse`. The limit is 256, set two orders of magnitude below anything a person writes and an order below the measured cliff, because the stack that matters is the smallest one the parser might run on — the LSP worker, a smaller container — not the machine it was measured on. Its own code rather than `TF010`: the `-` is legal exactly where it is written, so calling it an unexpected token would put a false word in the only sentence the reader gets. The message names the limit rather than a mistake, because at this depth the author is a generator and what a generator needs is the number. | a `let` whose value carries 300 unary minuses → `` too many nested `-` signs `` |
| `TF076` | Checker (config, `M147f`, `M147-07`, D647): **a `header "X" is "Y" for <service>` naming a service no `env` declares.** The scope clause narrows a header to one service, so a name matching nothing narrows it to *nothing*: `resolveConfig` copies the name through verbatim and the interpreter sets the header only where the clause is absent or matches the step's own service, so the header is attached to no request at all — `tflw check` clean, exit 0, run green, and every request going out without the header the config says it carries. This is the shape `TF074` was spent on for `session ... for env` one row earlier, in a construct that had already shipped. **Checked against the union of every service declared in the file, `defaults` and all envs, not against the active env**, because a header in `defaults` may legitimately scope to a service only one env declares and a per-env rule would reject a correct config. That catches every typo at zero false positives and answers in the editor on `tflw.config` alone; what it under-approximates is a header scoped to a service that exists only in an env the header never applies to, which is a decision `TF026`'s resolve-time position would have to make. Its own code rather than `TF026`: that one reads *not declared in the active env*, a claim about a resolution this check never performs. | a header scope clause naming a service this config does not declare → `unknown service "shp"` |
| `TF077` | Checker (`M156a`, D774-D777): **an `env(NAME)` that no `require env` line declares.** The run-start gate only ever protected the names the author remembered to write down, so an undeclared reference died mid-suite with `require env` present and correct — which is the failure the manifest's own summary promised it prevented. The two rules compose into that promise and neither gives it alone: every reference is declared (here, at check time), every declaration is satisfied before the first request (`cli.ts`, at run start). **An error, and the severity is settled by this checker's own precedent.** Every *referenced a name nothing declares* rule here is an error — `as admin` with no such session, `api orders` with no such service, `{foo}` unbound, `{prcie}` not a column — while all four warnings are predictions (`TF043`, `TF055`) or explicit not-necessarily-mistakes (`TF058`). `D147`'s test puts this on the error side: the reference is in the source and the declaration is in the config, so it is an **observation**. **`TF057` is not the counter-example it looks like** — it warns because its check is scoped to one env and another may declare the allowlist, and `require env` is a top-level directive flattened env-independently, so that escape does not exist. **Whether the variable is *set* is deliberately not asked**: that is the prediction, and `tflw check` reports it as an exit-code-neutral note instead (`D779`). **Checked over `tflw.config` as well as every test file**, and over the whole config rather than the active env's slice, since the declaration set has no env scope. The honest cost, recorded rather than argued away: the rest of the family refuse a suite that *cannot* run and this one refuses a suite that *can* — an undeclared `env(FOO)` works whenever `FOO` happens to be set. *Runs fine* is a property of the shell, not of the suite. | a secret read by a test that the config never declares → `` no `require env` line declares it `` |
| `TF078` | Checker (`M156b`, D778): **`{env(NAME)}` written inside a string literal**, which this language has never interpolated. `env(NAME)` is a *value*; `parseStringParts` rejects it as a `{ref}` path because parentheses are not path syntax, so the braces stay literal text and the request ships them verbatim — measured against a loopback echo server, a `header "X" is "{env(TOKEN)}"` sends the twelve characters `{env(TOKEN)}` and the run fails as a 401 with nothing anywhere naming the cause. It is the commonest spelling of the mistake `TF077` exists to catch and `TF077` cannot see it, because a braced `env()` is not a reference. **A warning, not an error**, deliberately: the contents of a string literal are the author's business and a checker that refuses one is guessing at intent; the hint names the repair, which is all a reader needs. **Anchored on the whole literal** rather than the occurrence inside it — `value` is the decoded text, so an escape earlier in the string shifts every later offset and a caret computed from it would point at the wrong character on exactly the strings hardest to read; the message names the variable instead. Revisit as an error if a suite is ever found shipping a live secret this way to a real endpoint. | a header written as a braced interpolation of a secret → `is literal text, not a secret` |
| `TF079` | Runtime (browser, `M159d`, `D802`): **an `accept dialog` or `dismiss dialog` that no dialog ever answered.** The arming waits for the *next* native dialog, wherever it fires; if none does before the test ends, the step did nothing and the test proves the same thing with the line deleted. Under the single slot this construct had before `D797` the case was not merely unreported but undetectable — a slot never read looks exactly like a slot read once — so the queue that fixed the two-dialog bug is what made the absence visible. **Reported once per unconsumed arming, at that arming's own line**, rather than as the count `D802` first drafted: armings carry provenance since `M159c`, and *armed minus raised* is the wrong arithmetic anyway, since an unarmed dialog is dismissed by default and still counted raised. **A warning, and only on an attempt that otherwise passed** (`D806b`): an absence has two causes this state cannot separate — a dead arming, or a body that threw before reaching the step that would have raised the dialog — and on a failed attempt the second is the likely one. `beforeunload` is the honest false positive: browsers raise it only after a real user gesture, so a headless run may correctly see none. | a `dismiss dialog` in a test where no dialog is ever raised — raised at run time, not by `tflw check` |
| `TF080` | Runtime (browser, `M159c`, `D801`): **an `accept dialog with "<text>"` whose answer reached a dialog that takes none.** Only a `prompt` has an input to fill; an `alert` has one button and a `confirm` has two, and Playwright *silently discards* `promptText` for both — so the test says an answer was given, the page never received one, and nothing anywhere reported the gap. That silent discard is the defect class `M159` exists to remove. **A warning, not an error, and the only code in this table that `tflw check` cannot emit:** which kind a page raises is a fact about the running page, and a page that raises an `alert` or a `prompt` depending on state is legitimate, so no static rule can decide it without being wrong about a correct test. The run's verdict is unchanged — the dialog is still accepted, exactly as a bare `accept dialog` would have accepted it. Its evidence is a runtime test rather than a probe, for the same reason. | `accept dialog with "Blue"` before a click that raises an `alert` — raised at run time, not by `tflw check` |
