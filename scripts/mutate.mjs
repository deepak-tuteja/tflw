...
// `M194` — the cost model that lived here (`SUITE_SECONDS`, measured per package on a 2-core
// runner and re-measured at M112, M148, M171c, M189b and M192) is gone with the runners' shards.
// `partition()` below deals by count, and a sweep runs on one machine whose K trees each hold a
// slice of every package; a stale number can no longer pack a shard it cannot pay for, because
// there is no number.
...
const REGISTRY = [
  ...
  // ---- U7: what the dogfood corpus taught the page --------------------------------------------
  {
    id: 'a-passing-hook-counts-as-a-test',
    milestone: 'm192',
    pkg: '@tflw/ui',
    file: 'packages/ui/src/live.ts',
  ...
  {
    id: 'every-exit-is-explained',
    milestone: 'm192',
    pkg: '@tflw/ui',
    file: 'packages/ui/src/format.ts',
    what: 'any exit code is taken as the report\'s own verdict — a 326/326 report whose process died at exit 2, or a cancel, shows as a clean run',
    find: "  if (run.status !== 'done' || run.signal !== null) return false;\n  switch (run.exitCode) {",
    replace: "  if (run.status !== 'done' || run.signal !== null) return true;\n  switch (run.exitCode) {",
  },
  {
    id: 'the-run-note-is-never-shown',
    milestone: 'm192',
    pkg: 'tflw',
  ...
  },
  {
    ...
    pkg: 'tflw',
    file: 'packages/ui/src/App.tsx',
    what: 'the live pane colours its verdict by failed tests rather than by the exit — a run that died before a test ran (exit 2) is drawn green',
    find: "live.end.exitCode === 0 ? 'ok' : 'fail'",
    replace: "failed > 0 ? 'fail' : 'ok'",
  },
  {
    id: 'an-unkept-run-has-no-row',
    milestone: 'm192',
    pkg: 'tflw',
    file: 'packages/ui/src/RunList.tsx',
    what: 'a run that ended with no report directory is dropped from the run list — a usage error vanishes the moment anything else is selected',
    find: "  const unkept = runs.filter((r) => r.status !== 'running' && r.kept === null);",
    replace: "  const unkept = runs.filter((r) => r.status !== 'running' && r.kept === null && false);",
  },
  {
    id: 'a-stale-report-is-kept-as-this-run',
    milestone: 'm192',
    pkg: 'tflw',
    file: 'packages/cli/src/ui-server.ts',
  ...
  },
  {
    ...
    what: 'the tag cloud opens folded for every project, the fixture\'s five included — the fold\'s threshold is what keeps a small project\'s tags in view',
  ...
  {
    id: 'the-menu-is-placed-against-an-infinite-window',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/ui/src/ContextMenu.tsx",
    what: "`D1145`'s clamp undone: the menu is placed against a window it believes is unbounded, so the four candidate positions never score an overshoot and a menu opened near an edge hangs off it. This is the defect the tooltip's `place()` exists to prevent, reached by lying to it about the viewport rather than by changing it.",
    find: "      { width: window.innerWidth, height: window.innerHeight },",
    replace: "      { width: 100000, height: 100000 },",
  },
  {
    id: 'escape-closes-the-menu-and-drops-focus',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/ui/src/ContextMenu.tsx",
    what: "`D1147` half-undone: `Escape` still closes the menu and no longer gives focus back to the row it was opened from, so a keyboard reader is returned to the top of the document. Silent in every rendering \u2014 the menu is gone either way.",
    find: "      if (e.key === 'Escape') { e.preventDefault(); close(true); return; }",
    replace: "      if (e.key === 'Escape') { e.preventDefault(); close(false); return; }",
  },
  {
    id: 'a-disabled-menu-item-says-nothing',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/ui/src/ContextMenu.tsx",
  ...
  {
    id: 'the-menu-names-the-open-file-not-the-clicked-row',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/ui/src/Sidebar.tsx",
    what: "`D1149` undone: the menu's subject becomes the file the tabs are facing rather than the row under the pointer. Every item still acts on the clicked row, so the header and the actions disagree \u2014 which is the exact confusion `D1149` refused when it rejected acting on the selection.",
    find: "            {...rowMenu({ kind: 'file', path: f.path }, f.path)}",
    replace: "            {...rowMenu({ kind: 'file', path: f.path }, openFile ?? f.path)}",
  },
  {
    id: 'the-reverse-index-cannot-see-a-use',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/cli/src/ui-refactor.ts",
    what: "`D1153`'s refusal narrowed: `use \"./helper.ts\"` stops counting as a reference, so a file a helper-using test depends on can be deleted and the importer is never named. The round never moves a `.ts` file, which is exactly why the gap would not show up in the half anyone exercises.",
    find: "    ...program.uses.map((node, index) => ({ what: 'use' as const, index, node })),",
    replace: "    ...[],",
  },
  {
    id: 'nothing-imports-anything',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/cli/src/ui-refactor.ts",
    what: "The reverse index answers *nobody* for every file. Delete stops refusing, move stops rewriting, and the menu stops disabling \u2014 three promises undone by one early return, which is `M168`'s rule about a check that passes because it was handed nothing.",
    find: "  const found: string[] = [];",
    replace: "  const found: string[] = []; if (target) return found;",
  },
  {
    id: 'delete-does-not-refuse-an-imported-file',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/cli/src/ui-refactor.ts",
    what: "`D1153` undone: the importers are still computed and still reported, and the refusal is no longer raised \u2014 so the delete goes through and every importer is left naming a file that is gone. The asymmetry with `planMove` is the thing being removed: a move repairs its blast radius and a delete cannot.",
    find: "  if (importers.length > 0) {",
    replace: "  if (false) {",
  },
  {
    id: 'the-recovery-question-is-about-the-project',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/cli/src/ui-server.ts",
    what: "`D1154` undone, and undone the exact way the grilling rejected: the dialog asks *is this a git repository* instead of *is this file committed*, so a file created five minutes ago and never committed is reported as recoverable. Confidently wrong in the commonest delete there is.",
    find: "      child = spawn('git', ['ls-files', '--error-unmatch', '--', path], { cwd: root, stdio: 'ignore' });",
    replace: "      child = spawn('git', ['rev-parse', '--is-inside-work-tree'], { cwd: root, stdio: 'ignore' });",
  },
  {
    id: 'a-move-is-not-validated-before-it-is-written',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/cli/src/ui-refactor.ts",
    what: "`D1151` undone: the edit set is no longer checked for parseability and formatting before the plan is returned, so a member that cannot be written is discovered mid-apply with earlier members already on disk \u2014 the half-renamed project with dangling imports that the whole all-or-nothing rule exists to prevent.",
    find: "  refusals.push(...validate(edits));",
    replace: "  refusals.push();",
  },
  {
    id: 'a-moved-file-keeps-its-old-relative-imports',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/cli/src/ui-refactor.ts",
    what: "`D1152`'s second direction undone: the importers are still re-pointed at the new location and the moved file's own `../` paths are left as they were, so moving a file between directories silently breaks what IT imports. Measured to fire on 4 of the sibling's 275 files \u2014 rare, and invisible until a run.",
    find: "  if (fromDir !== toDir) {",
    replace: "  if (false) {",
  },
  {
    id: 'a-draft-does-not-follow-its-file',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/ui/src/App.tsx",
    what: "`D1155` undone: a rename drops the old path's draft and does not re-key it, so pending edits are stranded under a path nothing opens again. Invisible on disk, invisible to the plan, and invisible to git \u2014 the one thing at risk that no server answer can see.",
    find: "              if (to !== null && held !== undefined) next.set(to, held);",
    replace: "              if (false && to !== null && held !== undefined) next.set(to, held);",
  },
  {
    id: 'duplicate-leaves-the-statements-behind',
    milestone: 'm218',
    pkg: 'tflw',
    file: "packages/ui/src/ComposeDoor.tsx",
    what: "`D1156` undone: the request is copied without the statements attached to it, so the copy lands between the original and its assertions and `body` re-points every one of them at the copy. The original stops being checked and the copy is checked twice \u2014 `D1138`'s hazard, measured at 19 of 19 requests.",
    find: "      const nodes = [request.node, ...request.attached.filter((a) => a.stepPath !== null).map((a) => a.node)];",
    replace: "      const nodes = [request.node];",
  },
];
...
/**
 * `mutations` dealt into `n` shards, deterministically, by count.
 *
 * `M194` — each package's mutations are cut into up to `n` contiguous slices and dealt round the
 * ring of shards from where the previous package stopped, so a package with fewer mutations than
 * shards lands on the shards the last one left out rather than on shard 1 every time. Every shard
 * holds about `1/n` of every package that has that many, and pays each package's baseline once —
 * the deal `M127`'s packer priced by a cost table for machines this sweep no longer runs on. On
 * one machine running all `n` shards at once the bound on the tail is one mutation of the slowest
 * suite, which no table could improve on.
 *
 * Deterministic at every step: slices are contiguous and in registry order, the package order is
 * the registry's, so the same registry always deals the same hands — which is what makes a shard's
 * contents reproducible from the shard number alone when one of them goes red.
 */
export function partition(mutations, n) {
  ...
  const bins = Array.from({ length: n }, () => ({ ms: [] }));
  let cursor = 0;
  for (const [, ms] of byPackage(mutations)) {
    const dealt = slices(ms, n);
    dealt.forEach((slice, i) => bins[(cursor + i) % n].ms.push(...slice));
    cursor += dealt.length;
  }
...
/** What a finished sweep says about its own clock — `D573`, printed on every run. `M194` took the
 *  soft budget off it: the budget was the runners' re-shard trigger, and the box driver reads
 *  `actualSeconds` out of the manifests instead. */
export function elapsedLine({ ms, shard }) {
  const what = shard ? `shard ${shard.index} of ${shard.of}` : 'this sweep';
  return `⏱ ${what} took ${formatElapsed(ms)}.`;
}
...
function main(argv = process.argv) {
  ...
  if (shard) {
    ...
    console.log(`shard ${shard.index} of ${shard.of} — ${selected.length} of ${MUTATIONS.length} mutation(s)\n`);
  }
  ...
  try {
    ...
    if (manifest) {
      writeFileSync(
        manifest,
        // `actualSeconds` (M148) stays: the box driver reads it back to pick its tree count, and a
        // number a run writes down is a number a later reader can compare. The modelled seconds
        // and the per-package baseline costs left with the cost model (`M194`).
        `${JSON.stringify(
          {
            shard: shard?.index ?? 1,
            of: shard?.of ?? 1,
