...
// ---------------------------------------------------------------------------
// `M194` — two halves ended here. `M137a` (`D449`) held the three-then-four copies of `ci.yml`'s
// shard count to each other, and `M148` (`M147-11`) tested the cost half: the re-shard trigger
// against `timeout-minutes`, `SUITE_SECONDS` against what a run measured, the balance probe over
// `SHARD_COUNT`. The sweep left the runners, `ci.yml` has no matrix, and the cost model is gone;
// the reassembly tests above are the half that survives, because a hole in a sweep is a hole on
// any machine.
