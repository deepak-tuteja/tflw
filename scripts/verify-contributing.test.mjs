...
const ABSENT_FROM_CI = [
  ...
  {
    local: 'TFLW_BIN=$PWD/packages/cli/dist/cli.cjs npm run regression --prefix ../testFlow-tests -- --parallel-groups',
    why: "`M195`/`D1011` — the sibling's regression sweep, the end-to-end layer, run at a milestone's close-out against the milestone's own build and recorded in its plan; the `TFLW_BIN` prefix is `M196`/`D1023` (`M196-01`: without it the sweep grades the sibling's vendored tarball, and `M195`'s own close-out did; `--parallel-groups` is `M196`'s successor `M197`/`D1026`, the four groups at once on one machine). It has a CI form, but in the *other* repository against the tflw its `main` last packed — days behind any tflw branch — and a cross-repo CI here would be the runner treadmill `M194` just left. So it is a rule, held by this row and by the plan's close-out line, not by a workflow",
  },
];
