...
const CLASSIFIED = [
  ...
  // --- ci.yml, job `mutations` (18 shards) ------------------------------------------------------
  ...
  },
  {
    wf: 'ci.yml',
    job: 'mutations',
    cmd: 'xvfb-run -a node scripts/mutate.mjs --shard=${{ matrix.shard }}/18 --manifest=shard-${{ matrix.shard }}.json',
    class: 'gate',
  ...
  { wf: 'ci.yml', job: 'mutation-controls', cmd: 'exit 1', class: 'ci-only', why: 'the second line of that annotation step' },
  {
    wf: 'ci.yml',
    job: 'mutation-controls',
    cmd: 'node scripts/verify-shards.mjs shards --of=18',
    class: 'ci-only',
    why: 'reads the eighteen uploaded shard manifests and asserts their union is the registry, and \u2014 since M148 \u2014 that the cost model they were packed by still describes what they cost. Locally the sweep is one process and covers itself',
  },
