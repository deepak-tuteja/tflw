...
const CLASSIFIED = [
  ...
  },
  {
    wf: 'ci.yml',
    job: 'mutations',
    cmd: 'xvfb-run -a node scripts/mutate.mjs --shard=${{ matrix.shard }}/12 --manifest=shard-${{ matrix.shard }}.json',
    class: 'gate',
  ...
  { wf: 'ci.yml', job: 'mutation-controls', cmd: 'exit 1', class: 'ci-only', why: 'the second line of that annotation step' },
  {
    wf: 'ci.yml',
    job: 'mutation-controls',
    cmd: 'node scripts/verify-shards.mjs shards --of=12',
    class: 'ci-only',
    why: 'reads the twelve uploaded shard manifests and asserts their union is the registry. Locally the sweep is one process and covers itself',
  },
